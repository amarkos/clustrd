summary.tune_cluspca <- function(object, ...) {
  x = object
  size = x$clusobj$size
  ...
  cat(paste("The best solution was obtained for ",k ," clusters of sizes ", paste(cs, collapse = ", ")," in ",d ," dimensions, for a cluster quality criterion value of ",round(x$critbest,3), ". Variables were ", centering, " and ", scaling,".", "\n", sep = ""))
  ...
  print(attc)
  cat("\nClustering vector:\n")
  print(x$clusobj$cluID)
  
  cat(paste("\nObjective criterion value:",round(x$clusobj$criterion,3),"\n"))
  
  cat("\nAvailable output:\n", 
      sep = "\n")
  #print(names(x$cluspcaobj))
  print(names(x))
  #invisible(x$cluspcaobj)
  invisible(x)
}